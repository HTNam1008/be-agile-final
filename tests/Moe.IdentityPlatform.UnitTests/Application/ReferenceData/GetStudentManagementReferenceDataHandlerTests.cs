...
public sealed class GetStudentManagementReferenceDataHandlerTests
{
    ...
    public async Task Handle_ReturnsStudentManagementOptionsWithoutAllPseudoValues()
    {
        ...
        response.StudentListFilters.ResidencyFilters
            .Should().BeEquivalentTo([
                new ReferenceDataOptionWithCode(nameof(AdminStudentResidencyFilter.SingaporeCitizen), "Singapore Citizen", "CITIZEN"),
                new ReferenceDataOptionWithCode(nameof(AdminStudentResidencyFilter.PermanentResident), "Permanent Resident", "PR"),
                new ReferenceDataOptionWithCode(nameof(AdminStudentResidencyFilter.Foreigner), "Foreigner", "VALID_PASS_HOLDER")
        ...
        response.StudentProfile.ResidencyCodes.Select(x => x.Value)
            .Should().Equal("CITIZEN", "PR", "VALID_PASS_HOLDER");
