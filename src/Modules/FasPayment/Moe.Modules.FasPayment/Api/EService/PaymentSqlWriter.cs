...
internal static class PaymentSqlWriter
{
    ...
    public static async Task<PaymentResult> PayBillAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return PaymentResult.Failure(PaymentErrors.InvalidPaymentMethod, ApiResponseCodes.BadRequest);
        }
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            if (bill is null)
            {
                return PaymentResult.Failure(PaymentErrors.BillNotFound, ApiResponseCodes.NotFound);
            }

            if (bill.OutstandingAmount <= 0m || bill.BillStatusCode is "PAID" or "CANCELLED")
            {
                return PaymentResult.Failure(PaymentErrors.BillAlreadySettled, ApiResponseCodes.Conflict);
            }
            ...
            if (paymentMethod == EducationAccountMethod)
            {
                if (account is null)
                {
                    return PaymentResult.Failure(PaymentErrors.AccountNotFound, ApiResponseCodes.NotFound);
                }

                if (account.CurrentBalance < bill.OutstandingAmount)
                {
                    return PaymentResult.Failure(PaymentErrors.InsufficientBalance, ApiResponseCodes.Conflict);
                }
            }
            ...
            return PaymentResult.Success(new PayBillResponse(
                paymentId,
                paymentPartId,
                bill.BillId,
                bill.BillNumber,
