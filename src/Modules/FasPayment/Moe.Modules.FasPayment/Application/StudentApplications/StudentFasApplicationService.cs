...
public sealed class StudentFasApplicationService(MoeDbContext db, ICurrentUser currentUser, IFasDocumentStorage storage, IFasDocumentScanner scanner)
{
    ...
    public async Task<object> ListSchemes(CancellationToken ct)
    {
        ...
        var schemes = await db.Set<FasScheme>().AsNoTracking().Where(x => x.StatusCode == "ACTIVE"&&applicable.Contains(x.Id))
            .OrderBy(x => x.StartDate).Select(x => new { id = x.Id, x.Name, shortDescription = x.Description,
                applicationStartDate = x.StartDate, applicationEndDate = x.EndDate,
                isOpenForApplication = x.StartDate <= today && x.EndDate >= today }).ToListAsync(ct);
    ...
    public async Task<object> SchemeDetail(long id, CancellationToken ct)
    {
        ...
        var courses = await db.Database.SqlQuery<CourseRow>($"""
            ...
            """).ToListAsync(ct);
        return new { scheme.Id, scheme.Name, scheme.Description, applicationStartDate = scheme.StartDate,
            applicationEndDate = scheme.EndDate, canApply = applicable.Contains(id)&&scheme.StartDate <= today && scheme.EndDate >= today,
            benefits = tiers, courses, appliesToAllCourses = courses.Count == 0 };
    }
    ...
    public async Task<object> CheckEligibility(EligibilityRequest request, CancellationToken ct)
    {
        ...
        foreach (var scheme in schemes)
        {
            ...
            foreach (var tier in tiers)
            {
                ...
                if (matched) { matches.Add(new { schemeId = scheme.Id, schemeName = scheme.Name, scheme.Description,
                    tierId = tier.Id, tierLabel = tier.Label, tier.SubsidyType, tier.SubsidyValue }); break; }
            }
        }
        return new { currentSchool = new { id = p.SchoolOrganizationId, name = p.SchoolName }, age,
            nationality = p.NationalityCode, monthlyHouseholdIncome = request.MonthlyHouseholdIncome,
            request.HouseholdMemberCount, parentNationalities, accountType, perCapitaIncome = decimal.Round(pci, 2), matchedSchemes = matches };
    }
    ...
    public async Task<object> CreateOrResumeDraft(CreateDraftRequest request, CancellationToken ct)
    {
        ...
        var active = await db.Set<FasActiveScheme>().AnyAsync(x => x.StudentPersonId == personId && x.StatusCode == "ACTIVE" && x.ActiveTo >= DateOnly.FromDateTime(now), ct);
        if (active) throw new InvalidOperationException("FAS.ACTIVE_SCHEME_EXISTS");
    ...
    public async Task<object> AdminApplications(string? status,long? schemeId,string? keyword,DateOnly? submittedFrom,DateOnly? submittedTo,int page,int pageSize,CancellationToken ct)
    {
        ...
        if(submittedFrom.HasValue){var d=submittedFrom.Value.ToDateTime(TimeOnly.MinValue);q=q.Where(x=>x.a.SubmittedAtUtc>=d);}if(submittedTo.HasValue){var d=submittedTo.Value.AddDays(1).ToDateTime(TimeOnly.MinValue);q=q.Where(x=>x.a.SubmittedAtUtc<d);}
    ...
    public async Task<object> Submit(long id, CancellationToken ct)
    {
        ...
        return await db.Database.CreateExecutionStrategy().ExecuteAsync(async () =>
        {
            ...
            if(await HasActiveScheme(person,ct))throw new InvalidOperationException("FAS.ACTIVE_SCHEME_EXISTS");
