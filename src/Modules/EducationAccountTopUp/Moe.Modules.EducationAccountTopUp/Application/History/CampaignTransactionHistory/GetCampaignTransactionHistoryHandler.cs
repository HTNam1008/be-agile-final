...
internal sealed class GetCampaignTransactionHistoryHandler(
    ...
    : IQueryHandler<GetCampaignTransactionHistoryQuery, PageResponse<CampaignTransactionHistoryItem>>
{
    public async Task<Result<PageResponse<CampaignTransactionHistoryItem>>> Handle(
        GetCampaignTransactionHistoryQuery query,
        CancellationToken cancellationToken)
    {
        ...
        {
            return Result<PageResponse<CampaignTransactionHistoryItem>>.Failure(accessResult.Error);
        }

        TransactionHistoryPage page = await transactionReader.GetCampaignTransactionsAsync(
            query.CampaignId,
            query.Filter,
            query.Page,
            query.PageSize,
        ...
        {
            return Result<PageResponse<CampaignTransactionHistoryItem>>.Success(
                new PageResponse<CampaignTransactionHistoryItem>(
                    [],
                    query.Page,
                    query.PageSize,
                    page.TotalCount));
        ...
        CampaignTransactionHistoryItem[] items = page.Items
    ...
    private static CampaignTransactionHistoryItem MapItem(
        ...
        IReadOnlyDictionary<long, TopUpStudentDisplaySummary> studentByPersonId)
    {
        ...
        return new CampaignTransactionHistoryItem(
            item.TransactionId,
            item.RunId,
            item.EducationAccountId,
            account is null
