...
public sealed class EmbeddedKnowledgeDocumentStore : IKnowledgeDocumentStore
{
    ...
    private readonly KnowledgeDocument[] _allDocs;

    public EmbeddedKnowledgeDocumentStore()
    {
        KnowledgeDocument[] fasChunks = LoadFasChunksFromAssembly();
        HashSet<string> packIds = [.. fasChunks.Select(x => x.Id)];
        _allDocs = [.. StaticDocs.Where(x => !packIds.Contains(x.Id)), .. fasChunks];
        ValidateKnowledgePacks(_allDocs);
    }

    public Task<IReadOnlyList<KnowledgeDocument>> GetAllAsync(CancellationToken ct)
        => Task.FromResult<IReadOnlyList<KnowledgeDocument>>(_allDocs);

    public Task<IReadOnlyList<KnowledgeDocument>> GetByDomainAsync(string domain, CancellationToken ct)
    {
        KnowledgeDocument[] filtered = _allDocs.Where(d =>
            d.Domain.Equals(domain, StringComparison.OrdinalIgnoreCase)).ToArray();
        return Task.FromResult<IReadOnlyList<KnowledgeDocument>>(filtered);
    }
    ...
    {
        Assembly assembly = typeof(EmbeddedKnowledgeDocumentStore).Assembly;
    ...
    internal static void ValidateKnowledgePacks(IEnumerable<KnowledgeDocument> docs)
    {
